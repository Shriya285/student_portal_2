...
def show_assignments():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""
        SELECT a.assignment_id, a.title, a.description, a.deadline, c.course_name, a.is_google_form, a.file_path,
...
def upload_assignment():
    ...
    assignment_type = st.radio("Assignment Type", ["File Upload", "Google Form"])
    
    if assignment_type == "File Upload":
        file = st.file_uploader("Choose a file (optional)", type=["pdf", "docx"])
    else:
        google_form_url = st.text_input("Google Form URL")

    if title and description and deadline and st.button("Upload", key="upload_assignment_button"):
        file_path = None
        if assignment_type == "File Upload" and file:
            file_path = f"assignments/{course['course_id']}_{file.name}"
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            with open(file_path, "wb") as f:
                f.write(file.getbuffer())
        elif assignment_type == "Google Form":
            file_path = google_form_url

        conn = get_db_connection()
        cursor = conn.cursor()
        try:
            cursor.execute("""
                INSERT INTO assignments (course_id, title, description, deadline, file_path, is_google_form)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (course['course_id'], title, description, deadline, file_path, assignment_type == "Google Form"))
